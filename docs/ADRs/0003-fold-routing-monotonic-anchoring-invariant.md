...
3. **The fold-routing map (this is the load-bearing artifact).**
   ...
   | `cross-verify` | pure-positive, monotone (SUM→SL-exp, `s=0`) | **Wave 1** — route wholesale |
   | `verification-strength` | pure-positive, monotone (weakest-link MIN over graph-fixed roots) | **Wave 1** — route wholesale |
   | `reach` | pure-positive (UNION + threshold); INV-13 display-only, never gates | **Wave 1** (or exclude as YAGNI) |
