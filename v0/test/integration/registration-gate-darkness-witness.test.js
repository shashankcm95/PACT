...
// PACT v0 -- registration-gate STRUCTURAL DARKNESS WITNESS (plans/39 §"Witnesses" control 12).
//
// Claim under proof: the plans/39 registration-provenance FILTER (trust/registration-gate.js) has EXACTLY ONE
// live consumer -- disjointPaths in trust/convert.js -- so its blast radius is contained to the ONE advisory
// readout. It arms nothing on its own (disarmed-by-default; the BEHAVIORAL disarmed-inertness proof is in
// registration-gate-convert.test.js item 10). This structural witness goes RED the instant a SECOND src module
// wires the filter into a live path (blast-radius creep, e.g. into read-gate for the wider verifiedRecords radius)
// -- the intended deliberate-update signal.
//
...
const MODULE_REL = 'trust/registration-gate.js';
...
test('precondition: trust/registration-gate.js exists (else the witness is vacuous)', () => {
...
test('CONTAINED: registration-gate is imported ONLY by trust/convert.js (its first + only live consumer)', () => {
  const importers = allSrcFiles(SRC)
    .filter((f) => !/registration-gate\.js$/.test(f))
